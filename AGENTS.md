...
- **Branch naming:** `fix/<slug>` for bugfixes, `feat/<slug>` (or `feature/<slug>`) for features, branched from `main`.
- **Issues** use the templates in `.github/ISSUE_TEMPLATE/`; tag the affected package with a `package: <name>` label (e.g. `package: data-types`) and include a confirmed reproduction.
- **PRs** target `main`, follow `.github/PULL_REQUEST_TEMPLATE.md`, and link the issue with `closes #N`. A good description has **Summary / Fix / Tests / Validation** sections (test + assertion counts, Rector result).
- **Do not push fixes directly to `main`** — go through a branch + PR even for small changes, so the issue/PR trail stays consistent.
