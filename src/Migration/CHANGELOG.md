...
- The `DbTracker` tracking table now has an `id` auto-increment column (driver-specific: `INTEGER PRIMARY KEY AUTOINCREMENT` on SQLite, `INT AUTO_INCREMENT PRIMARY KEY` on MySQL/MariaDB, `INTEGER GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY` on PostgreSQL) used to replay applied migrations in their exact application order, and `migration_id` is now a `UNIQUE` column instead of the primary key. Tables created by a previous (unreleased) iteration without this column keep working: `DbTracker` detects the missing `id` column and falls back to ordering by `applied_at` then `migration_id` (deterministic, but not guaranteed to be the real application order)
