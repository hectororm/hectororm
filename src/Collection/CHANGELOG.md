...
- `LazyCollection::chunk()` now throws a `ValueError` for a length `<= 0`, consistently with `array_chunk()` and `Collection::chunk()` (it previously built chunks of a single element)
